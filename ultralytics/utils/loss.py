...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        #self.conv_loss = SparsityCriterion(-1,300)
    ...
    #def __call__(self, preds, batch,meta):
...
class SparsityCriterion(nn.Module):
    '''
    定义稀疏性损失，包含两部分：
    - 网络损失：整个网络使用的计算预算与目标之间的均方误差（MSE）
    - 块损失：块中的稀疏性（使用的FLOPS百分比在0到1之间）必须在上限和下限之间。
      这个损失是逐步减小的。
    '''
    ...
    def forward(self, meta):
        ...
        返回：
        - loss_network + loss_block：网络损失和块损失的总和
        '''
        ...
        for i, mask in enumerate(meta['masks']):
            ...
            # 计算上限和下限损失
            loss_block += max(0, layer_perc - upper_bound) ** 2  # 上限
            loss_block += max(0, lower_bound - layer_perc) ** 2  # 下限
