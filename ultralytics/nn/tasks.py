...
from .backbone.MobileNetv4 import mobilenetv4_small
...
# folder1/folder2/folder3/file2.py
    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            for m in self.model.modules():
                if isinstance(m, (Conv, Conv2, DWConv,Conv5)) and hasattr(m, "bn"):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    if act:
        ...
        Conv5.default_act = eval(act)  # redefine default activation, i.e. Conv.default_act = nn.SiLU()
        if verbose:
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fCIB,C5f,man,C3STR,FEM}:
